...
def _trace_executor_inputs(*args: Any, **kwargs: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "task_complexity": getattr(
            getattr(invocation, "task_complexity", None), "value", None
        ),
        "group_count": len(getattr(grouped_plan, "action_groups", []) or []),
        "toolbox_name": getattr(toolbox, "name", None),
        "tool_count": len(getattr(toolbox, "tools", []) or []),
    }


def _trace_executor_outputs(output: Any) -> dict[str, Any]:
    if output is None:
